...
CircleArea(6);
RectangularArea(4, 5);
RParallelepipedArea(4,5,6);
CircleInsideTriangle(4, 5, 6, 3);

static void CircleArea(double r, int p = 3)
{
...
static void RectangularArea(int a, int b)
{
    int S = a*b;

    Console.WriteLine($"Düzbucaqlının sahəsi: {S}");
}
static void RParallelepipedArea(int a,int b,int c)
{
...
static void CircleInsideTriangle(int a, int b, int c ,int r)
{
    int p = (a + b + c) / 2;
    double S = p * r; 
